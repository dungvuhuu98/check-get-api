...
Link = "http://10.200.39."
Range = [24,25,24,25,24,25]
Filename = 'list_endpoint/endpoint_v2.txt'
Ascii_banner = pyfiglet.figlet_format("Test BO API-V2")

def check_log():
    list = []
    with open('log/nohup_v2.log', 'r', encoding='utf-8') as file:
        countok = 0
        countf = 0
        for line in file:
...
if __name__ == '__main__':
    print(Ascii_banner)
    print('Test APIv2 will start in 2s ...')
    time.sleep(2)
    logging.basicConfig(filename='log/nohup_v2.log', format='%(asctime)s - %(filename)s:%(lineno)d\t %(message)s', filemode='w', level=logging.DEBUG) # filemode='w': ghi de log cu
    try:
